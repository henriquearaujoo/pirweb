export const MENU = [

    {
        'id': 1,
        'title': 'USUÁRIO',
        'route': '',
        'routes': ['/', '/usuarios', '/registro', '/detalhes', '/perfis'],
        'read': false,
        'iClass': 'fa fa-user',
        'category': 'parent',
        'hasChild': true,
        'parent': 0
    },
    {
        'id': 2,
        'title': 'REGISTRO',
        'route': 'usuarios',
        'routes': ['/usuarios', '/registro', '/detalhes'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 1
    },
    {
        'id': 3,
        'title': 'PERFIL',
        'route': 'perfis',
        'routes': ['/perfis'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 1
    },
    {
        'id': 4,
        'title': 'LIVRO',
        'route': '',
        'routes': ['/', '/capitulos', '/dashboard'],
        'read': false,
        'iClass': 'fa fa-file-text',
        'category': 'parent',
        'hasChild': true,
        'parent': 0
    },
    {
        'id': 5,
        'title': 'CAPÍTULOS',
        'route': 'capitulos',
        'routes': ['/capitulos', '/dashboard'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 4
    },
    {
        'id': 6,
        'title': 'AGENTE',
        'route': '',
        'routes': ['/', '/agente-dashboard', '/registro', '/detalhes', '/localizacao',
                   '/agente-visita', '/historico', '/agente-desempenho', '/agentes-mapa'],
        'read': false,
        'iClass': 'fa fa-user',
        'category': 'parent',
        'hasChild': true,
        'parent': 0
    },
    {
        'id': 7,
        'title': 'REGISTRO',
        'route': 'agente-dashboard',
        'routes': ['agente-dashboard', '/registro', '/detalhes', '/localizacao'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 6
    },
    {
        'id': 8,
        'title': 'HISTÓRICO DE VISITAS',
        'route': 'agente-visita',
        'routes': ['/agente-visita', '/historico'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 6
    },
    // {
    //     'id': 9,
    //     'title': 'GRÁFICO DE DESEMPENHO',
    //     'route': 'agente-desempenho',
    //     'routes': ['/agente-desempenho'],
    //     'read': false,
    //     'iClass': 'fa fa-circle-o',
    //     'category': 'child',
    //     'hasChild': false,
    //     'parent': 6
    // },
    {
        'id': 10,
        'title': 'MAPA DOS AGENTES',
        'route': 'agentes-mapa',
        'routes': ['/agentes-mapa'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 6
    },
    {
        'id': 11,
        'title': 'REGIONAL',
        'route': '',
        'routes': ['/', '/regionais', '/comunidades', '/registro', '/detalhes'],
        'read': false,
        'iClass': 'fa fa-home',
        'category': 'parent',
        'hasChild': true,
        'parent': 0
    },
    {
        'id': 12,
        'title': 'REGISTRO',
        'route': 'regionais',
        'routes': ['/regionais', '/registro', '/detalhes'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 11
    },
    {
        'id': 13,
        'title': 'COMUNIDADES',
        'route': 'comunidades',
        'routes': ['/comunidades', '/registro', '/detalhes'],
        'read': true,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 11
    },
    // {
    //     'id': 14,
    //     'title': 'UC',
    //     'route': 'unidades',
    //     'routes': ['/unidades', '/registro', '/detalhes'],
    //     'read': true,
    //     'iClass': 'fa fa-circle-o',
    //     'category': 'child',
    //     'hasChild': false,
    //     'parent': 11
    // },
    {
        'id': 15,
        'title': 'FAMÍLIA',
        'route': '',
        'routes': ['/', '/gestantes', '/detalhes', '/registro',
                   '/familias', '/criancas'],
        'read': false,
        'iClass': 'fa fa-users',
        'category': 'parent',
        'hasChild': true,
        'parent': 0
    },
    {
        'id': 16,
        'title': 'REGISTRO',
        'route': 'familias',
        'routes': ['/familias', '/registro', '/detalhes'],
        'read': true,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 15
    },
    {
        'id': 17,
        'title': 'GESTANTES',
        'route': 'gestantes',
        'routes': ['/gestantes', '/registro', '/detalhes'],
        'read': true,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': true,
        'parent': 15
    },
    {
        'id': 18,
        'title': 'CRIANÇAS',
        'route': 'criancas',
        'routes': ['/criancas', '/registro', '/detalhes'],
        'read': true,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 15
    },
    {
        'id': 19,
        'title': 'FORMULÁRIO',
        'route': '',
        'routes': ['/', '/registro', '/formularios', '/detalhes'],
        'read': false,
        'iClass': 'fa fa-list-alt',
        'category': 'parent',
        'hasChild': true,
        'parent': 0
    },
    {
        'id': 20,
        'title': 'REGISTRO',
        'route': 'formularios',
        'routes': ['/formularios', '/registro', '/detalhes'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 19
    },
    {
        'id': 21,
        'title': 'CONSTRUTOR',
        'route': 'formularios/construtor',
        'routes': ['/formularios/construtor'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 19
    },
    {
        'id': 22,
        'title': 'RELATÓRIOS',
        'route': '',
        'routes': ['/', '/relatorios'],
        'read': false,
        'iClass': 'fa fa-line-chart',
        'category': 'parent',
        'hasChild': true,
        'parent': 0
    },
    {
        'id': 23,
        'title': 'REGISTRO',
        'route': 'relatorios',
        'routes': ['/relatorios'],
        'read': false,
        'iClass': 'fa fa-circle-o',
        'category': 'child',
        'hasChild': false,
        'parent': 22
    },
];

